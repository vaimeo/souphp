...
$str=$str."\n if($"."_POST['add'])
{
	...
	$"."data_manupuator->add($tableindex);
...
if($"."_POST['update'])
{
	$"."data_manupuator->_files=$"."_FILES;
	$"."data_manupuator->_post=$"."_POST;
	$"."data_manupuator->update($tableindex);
...
<div class=\"footgrid\"><input type=\"button\"  value=\"submit\"><input type=\"submit\"  id=\"add\"  value=\"add\" name=\"add\"><input type=\"submit\"  style=\"display:none\" id=\"update\"  value=\"update\" name=\"update\"></div>
	...
	if(!file_exists("adminPages/".$tableName.".php"))
	{
		touch("adminPages/".$tableName.".php");
	}
	
	
	file_put_contents("adminPages/".$tableName.".php",$str);
...
{
	?>
    Form created <a href="adminPages/<?php echo $tableName;?>.php">view page</a>
    <?php
}
?>
</td>
